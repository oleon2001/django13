...
class BLURequestHandler(SocketServer.BaseRequestHandler ):
	def UnpackPos(self,data):
		...
		return {'date': dt,'pos': Point(lon,lat),'speed': speed}
	...
	def Ping(self):
		pos = self.UnpackPos(self.data[5:18])
		...
		self.socket.sendto(response, self.client_address)
		print >> self.stdout, "Got PING w/position from ", self.avl.imei
...
if __name__ == "__main__":
	try:
		server = SocketServer.ThreadingUDPServer(('', 50000), BLURequestHandler)
		#server = SocketServer.UDPServer(('', 50000), BLURequestHandler)
		print "_"*80
